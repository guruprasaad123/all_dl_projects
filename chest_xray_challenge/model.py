...
class Inception3(nn.Module):
  ...
  def __init__(self, num_classes=10, aux_logits=True, training=True, transform_input=True):
...
if __name__ == '__main__':
    ...
    model = inception_v3(training=True,aux_logits=True,num_classes=2)
    ...
    print('Dim : {}'.format(image.shape))

    output , aux_ops_1 = model(image.T)

    print('output : {}'.format(output))

    cross_entropy_loss = nn.CrossEntropyLoss()

    loss = cross_entropy_loss(output, torch.tensor([label]))

    print('argmax : {}'.format(output.argmax(dim=1) ))
